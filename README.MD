...
```
$ go get github.com/marinogiaco/odfe-monitor-cli/
```

From binary to `./bin/odfe-monitor-cli`:

```
$ curl -sfL https://raw.githubusercontent.com/marinogiaco/odfe-monitor-cli/master/godownloader.sh | bash
```

From binary to `/usr/local/bin/odfe-monitor-cli`:

```
$ curl -sfL https://raw.githubusercontent.com/marinogiaco/odfe-monitor-cli/master/godownloader.sh | bash -s -- -b /usr/local/bin
```
